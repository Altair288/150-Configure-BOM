...
import { createSeed, validateStore, validateDefinition, validateValue, resolveDefinition, publishDefinition, copyContext, contextReferences, definitionUsage } from "../webapp/model/configuration.ts";
...
test("context copy isolates local definitions, products and profile but reuses enterprise IDs", () => {
  ...
  for (let i = 0; i < original.length; i++) {
    ...
    assert.equal(copied[i].featureDefinitionId === source.id, source.sourceType === "Enterprise Library");
  }
  assert.deepEqual(validateStore(store), []);
});
...
test("optional false is a valid boolean and FWD plus Offroad are independently valid", () => {
  ...
  assert.deepEqual(validateValue(store.definitions.find(d => d.code === "DRIVE_TYPE"), "FWD"), []);
  assert.deepEqual(validateValue(store.definitions.find(d => d.code === "PACKAGES"), ["OFFROAD_PACKAGE"]), []);
});
